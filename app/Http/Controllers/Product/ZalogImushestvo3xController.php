...
use App\Model\ContractTrilateralPropertyPledge;
...
class ZalogImushestvo3xController extends Controller
{
    ...
    public function create()
    {
        ...
        $specification = Specification::where('key', '=', 'S_PPIT')->get()->first();

        $contract = new Contract();
        $contract_trilateral_property_pledge = new ContractTrilateralPropertyPledge();
        ...
        if (isset($old_data['properties'])) {
            foreach ($old_data['properties'] as $item) {
                $contract_trilateral_property_pledge->properties[] = new Property();
            }
        }
        ...
        return view('products.zalog.imushestvo3x.form', [
            'beneficiary' => new Beneficiary(),
            'block' => false,
            'client' => new Client(),
            'contract' => $contract,
            'contract_trilateral_property_pledge' => $contract_trilateral_property_pledge,
            'policy' => new Policy(),
        ]);
    }
    ...
    public function store(Request $request)
    {
        $request->validate(array_merge(
            ...
            Client::$validate,
            Contract::$validate,
            ContractTrilateralPropertyPledge::$validate,
            [
        ...
        $contract_trilateral_property_pledge = ContractTrilateralPropertyPledge::create($request['contract_trilateral_property_pledge']);
        ...
        $contract_data['model_type'] = ContractTrilateralPropertyPledge::class;
        $contract_data['model_id'] = $contract_trilateral_property_pledge->id;
        ...
        if ($request['properties']) {
            $contract_trilateral_property_pledge->properties()->createMany($request['properties']);
        }
        ...
        $contract_trilateral_property_pledge_file_types = [
            ContractTrilateralPropertyPledge::FILE_OTHER_DOCUMENT,
            ContractTrilateralPropertyPledge::FILE_PASSPORT,
            ContractTrilateralPropertyPledge::FILE_REFERENCE,
            ContractTrilateralPropertyPledge::FILE_TREATY,
        ];

        $contract_files = [];
        $contract_trilateral_property_pledge_files = [];
        if (isset($request['files'])) {
            foreach($request['files'] as $type => $file_collection) {
                if (in_array($type, $contract_trilateral_property_pledge_file_types)) {
                    foreach ($file_collection as /* @var $file_item \Illuminate\Http\UploadedFile */ $file_item) {
                        $contract_trilateral_property_pledge_files[] = [
                            ...
                            'path' => Storage::putFile('public/contract_trilateral_property_pledge', $file_item),
                        ];
                    }
                } else {
        ...
        $contract_trilateral_property_pledge->files()->createMany($contract_trilateral_property_pledge_files);
     ...
     *
     * @param  \App\Model\Contract $zalog_imushestvo3x
     * @return \Illuminate\Http\Response
     */
    public function show(Contract $zalog_imushestvo3x)
    {
        $contract = $zalog_imushestvo3x;
        ...
        return view('products.zalog.imushestvo3x.form', [
            ...
            'contract' => $contract,
            'contract_trilateral_property_pledge' => $contract->contract_model,
     ...
     * @return \Illuminate\Http\Response
     */
    public function edit(Contract $zalog_imushestvo3x)
    {
        $contract = $zalog_imushestvo3x;
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function update(Request $request, Contract $zalog_imushestvo3x)
    {
        $request->validate(array_merge(
            Beneficiary::$validate,
            Client::$validate,
        ...
        $contract_trilateral_property_pledge = $contract->contract_model;
        $contract_trilateral_property_pledge->fill($request['contract_trilateral_property_pledge']);
        $contract_trilateral_property_pledge->save();
        ...
        if ($request['properties']) {
            ...
            foreach($request['properties'] as $property_data) {
                $property = Property::where('model_type', '=', ContractTrilateralPropertyPledge::class)
                                    ->where('model_id', '=', $contract_trilateral_property_pledge->id)
                ...
                if ($property) {
                    ...
                    $property_data['model_type'] = ContractTrilateralPropertyPledge::class;
                    $property_data['model_id'] = $contract_trilateral_property_pledge->id;
            ...
            $properties = Property::where('model_type', '=', ContractTrilateralPropertyPledge::class)
                                  ->where('model_id', '=', $contract_trilateral_property_pledge->id)
        ...
        if (isset($request['files'])) {
            foreach($request['files'] as $type => $file_collection) {
                if (in_array($type, $contract_trilateral_property_pledge_file_types)) {
                    foreach ($file_collection as /* @var $file_item \Illuminate\Http\UploadedFile */ $file_item) {
                        foreach($contract_trilateral_property_pledge->getFiles($type) as $old_file) {
                            $old_file->delete();
                        }
     ...
     * @throws \Exception
     */
    public function destroy(Contract $zalog_imushestvo3x)
    {
        $contract = $zalog_imushestvo3x;
