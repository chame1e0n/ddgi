...
(function () {
  ...
  jQuery(function ($) {
    var TranslationField = function TranslationField(options) {
      this.el = options.el;
      ...
      this.buildGroupId = function () {
        ...
        if (idBits.length === 3) {
          ...
          idPrefix = idBits[0] + '-' + idBits[1] + '-' + idBits[2] + '-' + idBits[3] + '-' + idBits[4] + '-' + this.origFieldname;
        } else if (idBits.length === 7) {
          // Handle generic inlines with model used by inline more than once
          idPrefix = idBits[0] + '-' + idBits[1] + '-' + idBits[2] + '-' + idBits[3] + '-' + idBits[4] + '-' + idBits[5] + '-' + this.origFieldname;
        }
    ...
    var TranslationFieldGrouper = function TranslationFieldGrouper(options) {
      ...
      $.each(groupedTranslations, function (groupId, lang) {
        ...
        $.each(lang, function (lang, el) {
          var container = $(el).closest('.form-row'),
              ...
              panel,
              tab; // Remove language and brackets from field label, they are
    ...
    function handleAddAnotherInline() {
      ...
      $('.mt').parents('.inline-group').not('.tabular').find('.add-row a').click(function () {
        var grouper = new TranslationFieldGrouper({
          $fields: $(this).parent().prev().prev().find('.mt').add( // Support django-nested-admin stacked inlines
          $(this).parent().prev('.djn-items').children('.djn-item').last().find('.mt'))
        });
        var tabs = createTabs(grouper.groupedTranslations); // Update the main switch as it is not aware of the newly created tabs

        MainSwitch.update(tabs); // Activate the language tab selected in the main switch

        MainSwitch.activateTab(tabs);
      });
    }

    var TabularInlineGroup = function TabularInlineGroup(options) {
      this.id = options.id;
      this.$id = null;
      this.$table = null;
      this.translationColumns = []; // TODO: Make use of this to flag required tabs
      ...
      this.initTable = function () {
        var self = this; // The table header requires special treatment. In case an inline
        // is declared with extra=0, the translation fields are not visible.

        var thGrouper = new TranslationFieldGrouper({
          $fields: this.$table.find('.mt').filter('input, textarea, select')
        });
        this.translationColumns = this.getTranslationColumns(thGrouper.groupedTranslations); // The markup of tabular inlines is kinda weird. There is an additional
      ...
      this.getTranslationColumns = function (groupedTranslations) {
        var translationColumns = []; // Get table column indexes which have translation fields, but omit the first
      ...
      this.getRequiredColumns = function () {
        var requiredColumns = []; // Get table column indexes which have required fields, but omit the first
        // one per group, because that's where we insert our tab container.
    ...
    function handleTabularAddAnotherInline(tabularInlineGroup) {
      tabularInlineGroup.$table.find('.add-row a').click(function () {
        var tabs = createTabularTabs(tabularInlineGroup.getGroupedTranslations($(this).parent().parent().prev().prev().find('.mt'))); // Update the main switch as it is not aware of the newly created tabs
    ...
    function createTabularTabs(groupedTranslations) {
      ...
      $.each(groupedTranslations, function (groupId, lang) {
        ...
        $.each(lang, function (lang, el) {
          ...
          $panel = $('<div id="' + tabId + '"></div>').append($container); // Turn the moved tds into divs
    ...
    var MainSwitch = {
      languages: [],
      $select: $('<select>'),
      init: function init(groupedTranslations, tabs) {
        var self = this;
        ...
        $.each(this.languages, function (idx, language) {
          self.$select.append($('<option value="' + idx + '">' + language.replace('_', '-') + '</option>'));
        });
        this.update(tabs);
      ...
      update: function update(tabs) {
        ...
        this.$select.change(function () {
          $.each(tabs, function (idx, tab) {
            try {
              //jquery ui => 1.10 api changed, we keep backward compatibility
      ...
      activateTab: function activateTab(tabs) {
        var self = this;
        $.each(tabs, function (idx, tab) {
          try {
            //jquery ui => 1.10 api changed, we keep backward compatibility
    ...
    if ($('body').hasClass('change-form')) {
      ...
      var grouper = new TranslationFieldGrouper({
        $fields: $('.mt').filter('input, textarea, select, iframe, div').filter(':parents(.tabular)').filter(':parents(.empty-form)')
      });
      MainSwitch.init(grouper.groupedTranslations, createTabs(grouper.groupedTranslations)); // Note: The add another functionality in admin is injected through inline javascript,
      ...
      $('div.inline-group > div.tabular').each(function () {
        ...
        MainSwitch.update(createTabularTabs(tabularInlineGroup.getAllGroupedTranslations()));
        $(document).ready(function () {
