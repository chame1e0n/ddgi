...
(function ($) {
  ...
  function drawSeries(plot, ctx, series) {
    ...
    for (var i = 0; i < points.length; i += ps) {
      var img = points[i],
          ...
          xaxis = series.xaxis,
          yaxis = series.yaxis,
          tmp; // actually we should check img.complete, but it
      ...
      x2 = xaxis.p2c(x2);
      y1 = yaxis.p2c(y1);
      y2 = yaxis.p2c(y2); // the transformation may have swapped us
  ...
  function processRawData(plot, series, data, datapoints) {
    if (!series.images.show) return; // format is Image, x1, y1, x2, y2 (opposite corners)
