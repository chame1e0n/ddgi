...
(function ($, initInlineTabularSelect, initInlineStackedSelect, initTextareaInline) {
  ...
  $.fn.formset = function (opts) {
    ...
    var maxForms = $("#id_" + options.prefix + "-MAX_NUM_FORMS").prop("autocomplete", "off"); // only show the add button if we are allowed to add more items,
    ...
    if ($this.length && showAddButton) {
      ...
      addButton.on('click', function (e) {
        ...
        nextIndex += 1; // Hide add button in case we've hit the max, except we want to add infinitely
        ...
        row.find("a." + options.deleteCssClass).on('click', function (e1) {
          e1.preventDefault(); // Remove the parent form containing this button:

          row.remove();
          nextIndex -= 1; // If a post-delete callback was provided, call it with the deleted form:
          ...
          $("#id_" + options.prefix + "-TOTAL_FORMS").val(forms.length); // Show add button again once we drop below max
  ...
  $.fn.tabularFormset = function (selector, options) {
    ...
    var alternatingRows = function alternatingRows(row) {
      $(selector).not(".add-row").removeClass("row1 row2").filter(":even").addClass("row1").end().filter(":odd").addClass("row2");
    };
