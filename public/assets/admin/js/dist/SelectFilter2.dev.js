...
(function ($) {
  ...
  window.SelectFilter = {
    init: function init(field_id, field_name, is_stacked) {
      ...
      choose_all.appendChild(createMaterialIcon('play_circle_filled')); // <ul class="selector-chooser">
      ...
      remove_link.appendChild(createMaterialIcon('play_circle_filled')); // <div class="selector-chosen">
      ...
      from_box.setAttribute('name', from_box.getAttribute('name') + '_old'); // Set up the JavaScript event handlers for the select box filter interface
    ...
    refresh_icons: function refresh_icons(field_id) {
      ...
      var to = $('#' + field_id + '_to'); // Active if at least one item is selected

      $('#' + field_id + '_add_link').toggleClass('active', SelectFilter.any_selected(from));
      $('#' + field_id + '_remove_link').toggleClass('active', SelectFilter.any_selected(to)); // Active if the corresponding box isn't empty
