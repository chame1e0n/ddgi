    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="card card-info">
                ...
                <div class="card-body">
                    <form role="form" action="{{ route('perestrahovaniya.update', $perestrahovaniya->id) }}"
                          method="post"
                ...
                (!is_null($perestrahovaniya->policy_blank) == false) ? 'none' : ''  }} " id="policy-type">
                                    ...
                                    <select name="policy_series_id" class="form-control select2" id="policy_series">
                                        ...
                                        @foreach($policySeries as $value)
                                            @if($perestrahovaniya->policy_series_id == $value->id)
                        ...
                        <div class="form-group" id="comments-form">
                            ...
                            <textarea id="comments" name="comments" class="textarea" placeholder="Place some text here"
                                      style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"> {{$perestrahovaniya->comments}} {{old('comments')}} </textarea>
                        </div>
                    ...
                    @if($canAdd)
                        <form method="post" action="{{ route('perestrahovaniya_overview.store') }}">
                            @csrf
                            <input type="hidden" value="{{ $perestrahovaniya->id }}" name="perestrahavniya_id">
                    ...
                    @endif
                    @foreach($perestrahovaniya->perestrahovaniyaOverview as $overview)
                        <form method="post" action="{{ route('perestrahovaniya_overview.update', $overview->id) }}">
                            @csrf
                            @method('put')
                            <div class="form-group">
                                <label>Статус претензии</label>
                            ...
                            <div class="form-group">
                                <label class="col-form-label"
                                       for="pretensii-final-settlement-date">Комментарий</label>
                                <div class="input-group mb-3">
                                    <input id="pretensii-final-settlement-date"
                            ...
                            <div class="form-group">
                                <label class="col-form-label"
                                       for="pretensii-final-settlement-date">Пользователь</label>
                                <div class="input-group mb-3">
                                    <input id="pretensii-final-settlement-date" name="user_name"
                                           @if(!empty($overview->user->name)) value="{{ $overview->user->name }}"
                                           @endif
                                           readonly type="text" class="form-control">
                                </div>
                            </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            getPolicies({{$perestrahovaniya->policy_series_id}});
